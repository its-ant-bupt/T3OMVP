...
class CriticNet(nn.Module):
    ...
    def build_inputs(self, states, histories, actions, device):
        ...
        states_ = []
        for state in states:
            states_ += [state.cpu().numpy() for _ in range(self.nr_agents)]
...
class COMALearner(A2CLearner):
    ...
    def __init__(self, params):
        ...
        self.critic_net = CriticNet(self.nr_actions, \
            self.nr_agents, self.global_input_shape, self.input_shape, self.max_history_length).to(self.device)
    ...
    def policy_update(self, minibatch_data, optimizer, random_agent_index=None):
        ...
        if True:
            ...
            for probs, next_probs, action, Q_values, next_Q, R in\
                ...
                next_Q = sum(next_probs*next_Q.cpu().numpy())
