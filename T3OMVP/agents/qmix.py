...
class QMIXNetwork(nn.Module):
    ...
    def forward(self, global_state, Q_values):
        if self.params["UPDeT"]:
            ...
            global_state = global_state.view(global_state.size(0), global_state.size(1), -1)
...
class QMIXLearner(VDNLearner):
    ...
    def make_mixer_neural_network(self):
        return QMIXNetwork(self.global_input_shape, self.nr_agents, params=self.params).to(self.device)
